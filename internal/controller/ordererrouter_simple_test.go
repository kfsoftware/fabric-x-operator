...
func TestOrdererRouterReconciler_NotFound(t *testing.T) {
	s := runtime.NewScheme()
	...
	fakeClient := fake.NewClientBuilder().
		WithScheme(s).
		WithStatusSubresource(&fabricxv1alpha1.OrdererRouter{}).
		Build()

	reconciler := &OrdererRouterReconciler{
		Client: fakeClient,
		Scheme: s,
	}
...
func TestOrdererRouterReconciler_BasicReconcile(t *testing.T) {
	s := runtime.NewScheme()
	require.NoError(t, fabricxv1alpha1.AddToScheme(s))
	require.NoError(t, corev1.AddToScheme(s))
