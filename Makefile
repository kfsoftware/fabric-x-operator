...
# kfsoft.tech/fabric-x-operator-bundle:$VERSION and kfsoft.tech/fabric-x-operator-catalog:$VERSION.
IMAGE_TAG_BASE ?= kfsoft.tech/fabric-x-operator
...
KIND_CLUSTER ?= fabric-x-operator-test-e2e
...
.PHONY: test-e2e-k3d
test-e2e-k3d: manifests generate fmt vet ## Run the e2e tests on an existing K3D cluster.
	CLUSTER_TYPE=k3d K3D_CLUSTER=$(or $(K3D_CLUSTER),k8s-hlf) go test ./test/e2e/ -v -ginkgo.v -timeout=30m
...
docker-buildx: ## Build and push docker image for the manager for cross-platform support
	...
	- $(CONTAINER_TOOL) buildx create --name fabric-x-operator-builder
	$(CONTAINER_TOOL) buildx use fabric-x-operator-builder
	- $(CONTAINER_TOOL) buildx build --push --platform=$(PLATFORMS) --tag ${IMG} -f Dockerfile.cross .
	- $(CONTAINER_TOOL) buildx rm fabric-x-operator-builder
	rm Dockerfile.cross
