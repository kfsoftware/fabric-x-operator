...
func (c *registerCmd) run(args []string) error {
	...
	certAuth, err := helpers.GetCertAuthByName(clientSet, clientSet, c.caOpts.Name, c.caOpts.NS)
	if err != nil {
		return err
	}
	var url string
...
func newCARegisterCmd(out io.Writer, errOut io.Writer) *cobra.Command {
	...
	f.StringVarP(&c.caOpts.NS, "namespace", "n", "default", "Namespace scope for this request")
