...
var (
	...
	projectImage = "kfsoftware/fabric-x-operator:e2e-test"
)
...
func TestE2E(t *testing.T) {
	...
	_, _ = fmt.Fprintf(GinkgoWriter, "Starting fabric-x-operator integration test suite\n")
...
var _ = BeforeSuite(func() {
	...
	cmd = exec.Command("kubectl", "wait", "--for=condition=Ready",
		...
		"-n", "fabric-x-operator-system",
		"--timeout=120s")
	_, err = utils.Run(cmd)
	ExpectWithOffset(1, err).NotTo(HaveOccurred(), "Controller manager pod not ready")
